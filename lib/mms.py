...
simple_sms_auth_token = os.environ.get("SIMPLE_SMS_TOKEN")
...
def send_plivo_text(number, mms_content, preview_image):
    ...
    number = "+1" + number
    payload = {"to": number, "imageUrl": preview_image, "message": mms_content}
    print(f"Sending MMS to number {number}: {payload}")
...
def main():
    send_simple_sms_text(
        "2102418565", "Testing testing", "https://jmattfong-halloween-public.s3.us-west-2.amazonaws.com/casey_256.jpg"
    )
